...
// de 28px. Na ArcDesk isso não é teoria: o tamanho do ícone é uma chave do
// gschema que vai de 32 a 128, então a MESMA capa é desenhada em quatro
// vezes mais área conforme o usuário mexe no slider.
//
 ...
 * painel de pasta) tem que chegar ao MESMO mini-ícone, e refazer a conta
 * lá seria duas fontes de verdade para a mesma geometria.
 */
...
/**
 * Miniatura da pasta: a "capa" que aparece na célula da área de trabalho.
 *
 ...
 * filhos dela) é a limpeza inteira — não há destroy() a chamar aqui. É por
 * isso que ela pode ser chamada de dentro de getDragActor(), que roda
 * depois do pushModal do dnd: não há recurso a vazar se algo der errado.
 *
...
export function createFolderPreview(apps, size) {
    ...
    //
    // A classe é só a clara: o tema escuro da ArcDesk é ADITIVO
    // (`arc-folder-tile arc-folder-tile-dark`), e quem sabe o tema é a
    // célula — pendurar a variante aqui exigiria um terceiro parâmetro numa
    // função que o contrato declara com dois.
    const tile = new St.Bin({
        style_class: 'arc-folder-tile',
        width: size,
        height: size,
        reactive: false,
        x_align: Clutter.ActorAlign.CENTER,
...
function _createTexture(app, size) {
    // Mesmo contrato do deskIcon: create_icon_texture() resolve o tema de
    // ícones (inclusive o fallback por wm_class) e só devolve null com um
    // .desktop sem ícone nenhum — e aí a vaga ainda precisa de algo do
